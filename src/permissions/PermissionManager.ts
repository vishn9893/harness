...
export class PermissionManager {
  constructor(private readonly root: string, private readonly settings: () => { reads: boolean; writes: boolean; terminal: boolean }) {}
  async approve(tool: AgentTool, args: unknown): Promise<boolean> {
    ...
    if (!dangerous && ((tool.name === 'write_file' && s.writes) || (tool.name === 'run_terminal' && s.terminal))) return true;
