...
export class ChatPanel {
  ...
  private constructor(
    ...
    private readonly actions: {
      ...
      addSkill(): void; addMcp(): void; history(): void; loadSession(id: string): void; deleteSession(id: string): void; togglePin(id: string): void; toggleAutoApprove(): void; toggleLightMode(): void; setAgentMode(mode: string): void; toggleAutoCollapseReasoning(): void;
    }
  ) {
    ...
    panel.webview.onDidReceiveMessage(async message => {
      ...
      if (message.type === 'setAgentMode') this.actions.setAgentMode(String(message.mode));
      if (message.type === 'toggleAutoCollapseReasoning') this.actions.toggleAutoCollapseReasoning();
    });
  ...
  setAgentMode(mode: string) { this.panel.webview.postMessage({ type: 'agentMode', mode }); }
  setAutoCollapseReasoning(enabled: boolean) { this.panel.webview.postMessage({ type: 'autoCollapseReasoning', enabled }); }
...
.msg{white-space:pre-wrap;margin:10px 0;padding:10px 12px;border:1px solid var(--border);border-radius:var(--radius);background:var(--accent);color:var(--accent-foreground)}.user{background:var(--primary);color:var(--primary-foreground)}.assistant{background:var(--card)}.tool,.status{font-size:12px}.error{background:#7f1d1d;color:#fee2e2}.reasoning{margin:10px 0;border:1px solid var(--border);border-radius:var(--radius);background:var(--field);color:var(--muted);font-size:12px}.reasoning summary{cursor:pointer;padding:8px 10px;font-weight:700;letter-spacing:.2px}.reasoning-content{padding:0 10px 9px;white-space:pre-wrap}
...
function add(cls,text){if(cls!=='assistant'){const d=document.createElement('div');d.className='msg '+cls;d.textContent=text;box.appendChild(d);d.scrollIntoView();return}const wrapper=document.createElement('div');wrapper.className='msg assistant';const source=String(text||''),pattern=/<(think|thinking|reasoning)>([\s\S]*?)<\/(?:think|thinking|reasoning)>/gi;let cursor=0,match;while((match=pattern.exec(source))){if(match.index>cursor){const answer=document.createElement('div');answer.textContent=plainText(source.slice(cursor,match.index));wrapper.appendChild(answer)}const details=document.createElement('details');details.className='reasoning';details.open=!autoCollapseReasoning;const summary=document.createElement('summary');summary.textContent='REASONING';const content=document.createElement('div');content.className='reasoning-content';content.textContent=plainText(match[2]);details.append(summary,content);wrapper.appendChild(details);cursor=pattern.lastIndex}if(cursor<source.length||!wrapper.childNodes.length){const answer=document.createElement('div');answer.textContent=plainText(source.slice(cursor));wrapper.appendChild(answer)}box.appendChild(wrapper);wrapper.scrollIntoView()}
